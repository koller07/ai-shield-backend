...
`SELECT u.id, u.email, u.name, u.role, u.password_hash, c.id AS company_id, c.name AS company_name, c.company_code, s.plan, s.status, s.billing_cycle, s.trial_ends_at, s.current_period_end FROM users u JOIN companies c ON c.id = u.company_id LEFT JOIN subscriptions s ON s.company_id = c.id WHERE u.email = $1 AND u.role = 'manager' ORDER BY s.created_at DESC LIMIT 1`,
[email]
);
...
try {
const { rows } = await pool.query(
`SELECT u.id, u.email, u.name, u.role, u.password_hash, c.id AS company_id, c.name AS company_name, s.status, s.trial_ends_at, s.plan FROM users u JOIN companies c ON c.id = u.company_id LEFT JOIN subscriptions s ON s.company_id = c.id WHERE u.email = $1 AND u.role = 'employee' ORDER BY s.created_at DESC LIMIT 1`,
[email]
);
...
try {
const { rows } = await pool.query(
`SELECT u.id, u.email, u.name, u.role, c.id AS company_id, c.name AS company_name, c.company_code, s.plan, s.status, s.billing_cycle, s.trial_ends_at, s.current_period_end FROM users u JOIN companies c ON c.id = u.company_id LEFT JOIN subscriptions s ON s.company_id = c.id WHERE u.id = $1 ORDER BY s.created_at DESC LIMIT 1`,
[req.user.userId]
);
...
return `<div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:40px 32px"> <h1 style="font-size:22px;font-weight:800;color:#0D1117;margin:0 0 12px"> 🛡️ Your 14-day trial has started. </h1> <p style="color:#3A4250;font-size:15px;line-height:1.65;margin:0 0 16px"> Hi ${name}, welcome to AI Shield. </p> <p style="color:#3A4250;font-size:15px;line-height:1.65;margin:0 0 24px"> Share this code with your team so they can activate the Chrome extension: </p> <div style="background:#F7F8FA;border:2px dashed #E3E8EF;border-radius:12px; padding:20px;text-align:center;margin-bottom:24px"> <div style="font-size:11px;color:#9CA3AF;font-weight:600;letter-spacing:.1em; text-transform:uppercase;margin-bottom:8px">Company Code</div> <div style="font-size:32px;font-weight:800;color:#0052CC;letter-spacing:.12em; font-family:monospace">${companyCode}</div> </div> <a href="https://getaishield.co/dashboard.html" style="display:inline-block;background:#0052CC;color:white;font-weight:600; padding:12px 24px;border-radius:8px;text-decoration:none;font-size:14px"> Open Dashboard → </a> </div>`;
}
